    ...
    def update_game_state(self):
        ...
        def check_collision(self, bullet, target):
            if bullet.direction == "UP":
                return bullet.x in range((target.x-target.size//2), (target.x + target.size//2)) and bullet.y in range(target.y, target.y + target.size)
            else:
                return bullet.x in range((target.x-target.size), (target.x + target.size)) and bullet.y in range(target.y-target.size, target.y + target.size)
